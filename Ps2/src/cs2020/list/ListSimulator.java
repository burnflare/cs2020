...
public class ListSimulator {
	...
	public static enum ListType {FIXED, MOVETOFRONT, MYFAST};
	...
	private static FixedLengthList getList(ListType type){
		if(type == ListType.FIXED)
			return new FixedLengthList(LISTSIZE);
		else if(type == ListType.MOVETOFRONT)
			return new MoveToFrontList(LISTSIZE);
		else
			return new MyFastList(LISTSIZE);
	}
	
	/**
	 * Method runExperiments
	 * @param Request list type
	 * This is basically what was provided to us in the main function
	 * I decided to encapsulate it inside a function of it's own so that
	 * I can call it multiple times with different list sent it.
	 */
	
	private static void runExperiments(ListType type){
		// Initialize the three experiments
		ListSimulator lRandom = new ListSimulator(TestType.RANDOM, getList(type));
		ListSimulator lUp = new ListSimulator(TestType.INCREASING, getList(type));
		ListSimulator lDown = new ListSimulator(TestType.DECREASING, getList(type));
		...
		for (i=0; i<ListSimulator.NUMQUERIES; i++){
			key = lRandom.chooseRandom();
			lRandom.search(key);
		}
				
		key = 0;
		for (i=0; i<ListSimulator.NUMQUERIES; i++){
			key = lUp.chooseRandom();
			lUp.search(key);
		}
				
		key = 0;
		for (i=0; i<ListSimulator.NUMQUERIES; i++){
			key = lDown.chooseRandom();
			lDown.search(key);
		}
		
		// Print out the results.
		System.out.println("Total cost random: " + lRandom.getTotal());
		System.out.println("Total cost increasing: " + lUp.getTotal());
		System.out.println("Total cost decreasing: " + lDown.getTotal());
	...
	static public void main(String[] args){
		/**
		 * Answer to 1.c.
		 * Looks like MyFastList performs best by a slight margin over MoveToFrontList.
		 * Just run this main function to see results printed to console
		 * The Move-To-Front heuristic makes a significant improvement in the
		 * efficacy of the search algorithm
		 */
		
		System.out.println("Experiment: FixedLenghtList");
		runExperiments(ListType.FIXED);
		System.out.println("\nExperiment: MoveToFrontList");
		runExperiments(ListType.MOVETOFRONT);
		System.out.println("\nExperiment: MyFastList");
		runExperiments(ListType.MYFAST);
